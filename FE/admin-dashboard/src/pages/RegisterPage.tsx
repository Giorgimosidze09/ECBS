...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await register({ username, password, role: 'customer' });
      history.push('/login');
    } catch (err: any) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #4a4e69 0%, #9a8c98 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ maxWidth: 400, width: '100%', background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(34,34,59,0.18)', padding: 36, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <h1 style={{ color: '#22223b', fontWeight: 800, fontSize: 32, marginBottom: 24 }}>Register</h1>
        <form onSubmit={handleSubmit} style={{ width: '100%' }}>
          ...
          <button type="submit" style={{ width: '100%', background: 'linear-gradient(90deg, #4a4e69 60%, #9a8c98 100%)', color: '#fff', border: 'none', borderRadius: 8, padding: '12px 0', fontWeight: 700, fontSize: 18, marginTop: 8, cursor: 'pointer', boxShadow: '0 2px 8px #c9ada7' }}>Register</button>
        </form>
        <div style={{ marginTop: 24, color: '#4a4e69', fontWeight: 500 }}>
          Already have an account?{' '}
          <Link to="/login" style={{ color: '#9a8c98', fontWeight: 700, textDecoration: 'underline' }}>Login</Link>
        </div>
      </div>
    </div>
  );
